...
Module Module1
    ...
    Private Const SW_HIDE As Integer = 1
    ...
    Sub ProcessMailItem()
        ...
        If TypeOf oMailItemSource Is Outlook.MailItem And oMailItemSource.Sent And Not SyncLabelExists() Then
            If Not OriginMailItemIsFoundedInDestination() Then
                Log("MailImtem is not founded in destination", True)
                CopyMailItem()
                Log("MailItem is copied", True)
            Else
                Log("MailImtem is founded in destination", True)
            End If
            SetSyncLabel()
            Log("MailItem is labeled", True)
        End If

        If sDeleteAfterSync = "Yes" And IsNumeric(iDeleteSyncedOlderThanDays) = True Then
            DeleteOldMailItem()
            Log("MailItem is deleted", True)
        End If
